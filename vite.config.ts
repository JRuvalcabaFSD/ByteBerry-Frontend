...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        '/api': {
          target: 'http://localhost:4002', // ✅ BFF directo
          changeOrigin: true,
          secure: false,
          ...
          configure: (proxy, _options) => {
            proxy.on('error', (err, _req, _res) => {
              ...
              console.log('💡 Asegúrate que el BFF esté corriendo en http://localhost:4002');
            });
            proxy.on('proxyReq', (_proxyReq, req, _res) => {
              console.log('→ Proxy:', req.method, req.url, '→', 'http://localhost:4002' + req.url);
            });
        ...
        '/auth': {
          ...
          changeOrigin: true,
          // NO reescribir path - mantener /auth/*
        },
      },
    },
